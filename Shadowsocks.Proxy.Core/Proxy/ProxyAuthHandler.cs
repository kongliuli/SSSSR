...
namespace Shadowsocks.Proxy.Core.Proxy
{
    public class ProxyAuthHandler
    {
        ...
        private void HandshakeReceive2Callback(IAsyncResult ar)
        {
            try
            {
                ...
                if (bytesRead >= 5)
                {
                    ...
                    // Validate ATYP per RFC 1928 Section 4
                    var atyp = _connetionRecvBuffer[3];
                    ...
                    // Validate address length does not exceed buffer capacity
                    if (size + ADDR_PORT_LEN > BufferSize)
                    {
                        throw new SocketException((int)SocketError.MessageSize);
                    }
        ...
        private void HandshakeReceive3Callback(int bytesRemain)
        {
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    // Validate port is non-zero per RFC 1928
                    if (_remoteHeaderSendBuffer.Length >= ADDR_PORT_LEN + 1)
                    {
                        var portIdx = _remoteHeaderSendBuffer.Length - ADDR_PORT_LEN;
                        var port = _remoteHeaderSendBuffer[portIdx] * 0x100 + _remoteHeaderSendBuffer[portIdx + 1];
                        if (port == 0)
                        {
        ...
        private void Connect()
        {
            ...
            handler.cfg.ProxyRuleMode = _config.ProxyRuleMode;
            handler.DnsClients = _config.DnsClients;
